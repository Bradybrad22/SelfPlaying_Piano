/*  
 * 61-Key Automatic Piano (C2~C7) using ESP32-C3 + 4×PCA9685 + UDP Trigger + Mic Lag Meter
 * ---------------------------------------------------------------------------------------
 * - 上电不播放；收到 UDP 文本里包含 WIFI_TRIGGER(默认 "S") 时，延迟 1 秒后播放一次（one-shot）
 * - 增加麦克风(Adafruit 1713/MAX9814)声音检测，统计“指令发出→声音检测”延迟
 * - LED 在理论应发声瞬间点亮，帮助校准 <40ms 误差
 *
 * 串口额外命令：
 *   TD   打印延迟统计(min/max/avg/count)
 *   TZ   清零统计
 *   TT x 设置声音触发阈值 (0-4095)
 */
...
struct NoteEvent;                       
extern const NoteEvent score[] PROGMEM; 
extern const size_t SCORE_LEN;          
...
volatile bool udpTriggerPending = false;   
...
/* ==================== 声音延迟测量相关 ==================== */
#define MIC_PIN        0      // 请按你的接线修改(ESP32-C3 ADC可用脚)
#define LED_PIN        1      // 板载或外接 LED
#define MIC_SAMPLE_INTERVAL_US  200   // 5kHz 采样
#define LAG_QUEUE_SIZE  16
#define LED_ON_MS       20    // LED 闪烁时间(提示应发声点)
volatile uint32_t lastMicSampleUs = 0;

struct LagItem {
  uint8_t  midi;
  uint32_t cmdUs;   // 发出击键指令的瞬间
  bool     waiting;
};
LagItem lagQueue[LAG_QUEUE_SIZE];

uint32_t lagCount = 0;
uint32_t lagSumMs = 0;
uint32_t lagMinMs = 0xFFFFFFFF;
uint32_t lagMaxMs = 0;

int micBaseline = 2048;
int micThresh   = 120;    // 可用 TT 命令调整
bool spikeArmed = true;   // 防止持续触发
uint32_t ledOffAt = 0;
...
inline bool mapMidi(uint8_t midi, KeyMap &km){
  ...
  if(driverIndex == 3 && channel > 12) return false;
...
/* ======== 延迟测量辅助 ======== */
void lagQueuePush(uint8_t midi, uint32_t tUs){
  for(int i=0;i<LAG_QUEUE_SIZE;i++){
    if(!lagQueue[i].waiting){
      lagQueue[i]={midi,tUs,true};
      return;
    }
  }
}

void recordLag(uint32_t lagMs){
  lagCount++;
  lagSumMs += lagMs;
  if(lagMs < lagMinMs) lagMinMs = lagMs;
  if(lagMs > lagMaxMs) lagMaxMs = lagMs;
}

void resetLagStats(){
  lagCount=0; lagSumMs=0; lagMinMs=0xFFFFFFFF; lagMaxMs=0;
}
...
void noteOn(uint8_t midi, uint32_t nowMs, uint32_t dur){
  midi = remapIfBad(midi);
  if(isBadKey(midi)) return;
  int k = keyIndex(midi); if(k<0) return;
  if(nowMs < keyRecoverUntil[k]){ if(inKeyTest) skipCount[k]++; return; }
  ...
  activeNotes[slot] = { midi, nowMs + hold, true };
  keyRecoverUntil[k] = nowMs + hold + KEY_RECOVERY_EXTRA_MS;
...
void handleSerial(){
  ...
  /* ---- 新增延迟统计命令 ---- */
    ...
    if(lagCount==0) Serial.println(F("[TD] No data yet."));
    else {
      float avg = (float)lagSumMs / lagCount;
      Serial.printf("[TD] count=%lu min=%lums max=%lums avg=%.2fms thr=%d\n",
        (unsigned long)lagCount,(unsigned long)lagMinMs,(unsigned long)lagMaxMs,avg,micThresh);
    }
  } else if(cmd=="TZ"){
    resetLagStats();
    Serial.println(F("[TZ] lag stats cleared."));
  } else if(cmd=="TT"){
    int v = rest.toInt();
    if(v<5) v=5; if(v>3000) v=3000;
    micThresh = v;
    Serial.printf("[TT] mic threshold=%d\n", micThresh);
...
void handleUDPTrigger() {
  ...
  if (cb > 0) {
    ...
    if (!playedOnce && strstr(packetBuffer, WIFI_TRIGGER)) {
      ...
      triggerAtMs = millis() + 1000;   // 延迟 1 秒启动
...
void sampleMic(){
  uint32_t nowUs = micros();
  if(nowUs - lastMicSampleUs < MIC_SAMPLE_INTERVAL_US) return;
  lastMicSampleUs = nowUs;

  int raw = analogRead(MIC_PIN);        // 0~4095
  // 简单滑动平均更新基线
  micBaseline = (micBaseline*31 + raw) >> 5;  // ~1/32 滤波
  int diff = abs(raw - micBaseline);

  if(spikeArmed && diff > micThresh){
    // 找到一个 spike，配对队列中最早的 waiting
    for(int i=0;i<LAG_QUEUE_SIZE;i++){
      if(lagQueue[i].waiting){
        uint32_t lagMs = (nowUs - lagQueue[i].cmdUs)/1000;
        recordLag(lagMs);
        lagQueue[i].waiting = false;
        Serial.printf("[LAG] midi=%u lag=%lums\n", lagQueue[i].midi, (unsigned long)lagMs);
        break;
      }
    }
    spikeArmed = false;
  } else if(!spikeArmed && diff < (micThresh/2)){
    // 退回滞环
    spikeArmed = true;
  }
}
...
void setup(){
  ...
  Serial.println(F("\n[Init] 61-Key Piano Controller + Lag Meter"));
  ...
  resetLagStats();
  memset(lagQueue,0,sizeof(lagQueue));

  Serial.println(F("[Ready] Commands: S P B V L T O W R X K KX KS KR KD F FX FS TD TZ TT <thr>"));
